        ...
        for (String s : list) {
            if (s.contains(".json")) {
                ...
                if (result.contains("install_id_str") && result.contains("device_id_str")) {
                    ...
                    String format = String.format("INSERT INTO douyin_device_iid (  device_id, iid,fail_reason )VALUES('%s', '%s','%s');",
                            parseObject.getString("device_id_str"), parseObject.getString("install_id_str"), DateUtil.formatDateTime(new Date()));
